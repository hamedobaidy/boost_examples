//============================================================================
// Name        : boost_calculate_integral.cpp
// Author      : Hamed Mohammadi
// Version     :
// Copyright   : Your copyright notice
// Description : Boost C++ libraries - calculating integrals example
//============================================================================
...
inline value_type integral(const value_type a, const value_type b,
		const value_type tol, function_type func) {
	unsigned n = 1U;
...
public:
	cyl_bessel_j_integral_rep(const unsigned N, const value_type& X) :
			n(N), x(X) {
	}
...
int main() {
	...
	const float j2_f = integral(0.0F, pi<float>(), 0.01F,
			cyl_bessel_j_integral_rep<float>(2U, 1.23F)) / pi<float>();

	const double j2_d = integral(0.0, pi<double>(), 0.0001,
			cyl_bessel_j_integral_rep<double>(2U, 1.23)) / pi<double>();

	const mp_type j2_mp = integral(mp_type(0), pi<mp_type>(), mp_type(1.0E-20),
			cyl_bessel_j_integral_rep<mp_type>(2U, mp_type(123) / 100))
			/ pi<mp_type>();

	// 0.166369
	std::cout << std::setprecision(std::numeric_limits<float>::digits10) << j2_f
			<< std::endl;
